...
public class PointSourceRAM implements PointSource {
    ...
    public PointSourceRAM(List<float[]> dataList) {
        if (dataList instanceof PointSourceRAM) {
            PointSourceRAM dataPSR = (PointSourceRAM) dataList;
            this.data = Arrays.copyOf(dataPSR.data, dataPSR.nextPoint);
            d = dataPSR.d;
            size = dataPSR.size;
            nextPoint = dataPSR.nextPoint;
        } else {
            d = dataList.get(0).length;
            size = dataList.size();
            nextPoint = 0;
            this.data = new float[d * size];
            int currentInsert = 0;
            for (float[] point : dataList) {
                if (point.length != d) {
                    throw new RuntimeException("All points need same dimensionality.");
                }
                for (int i = 0; i < d; i++) {
                    this.data[currentInsert++] = point[i];
                }
            }
        }
    }
