...
public final class SkylineAlgorithm_RDF_BNL extends AbstractSkylineAlgorithm {
    ...
    public SkylineAlgorithm_RDF_BNL(PointComparator_RDF comparator) {
    ...
    public synchronized List<float[]> compute(PointSource data) {
        ...
        int d = data.getD();
        PointList.ListOrder listOrder;
        switch (bnlWindowPolicy) {
            case KeepSorted:
                listOrder =  PointList.ListOrder.SortedByVolume;
                break;
            case BubbleUp:
                listOrder =  PointList.ListOrder.BubbleUp;
                break;
            case BubbleUpSimple:
                listOrder =  PointList.ListOrder.BubbleUpSimple;
                break;
            case MoveToFront:
                listOrder =  PointList.ListOrder.MoveToFront;
                break;
            default:
                listOrder =  PointList.ListOrder.Unsorted;
                break;
        }
     ...
     */
    public  boolean bnlOperation(PointList window, float[] dataPoint, boolean insertionAllowed, BNLProfiler profiler) {
        int cpuCost = 0;
        int insertions = 0;
        int deletions = 0;
        int moves = 0;
        ...
        while (windowIter.hasNext()) {
            float[] currentData = windowIter.nextDirect();
            PointRelationship dom = comparator.compare(currentData, dataPoint);
            cpuCost++;
            switch (dom) {
...
//	    }
    ...
    public String toString() {
        ...
        buf.append("BNL-RDF");
        buf.append(" (Policy: " + bnlWindowPolicy + ") + " + comparator.toString());
        return buf.toString();
    }
