...
public class DumpFileGenerator extends AbstractDataGenerator {
    ...
    public float[] generate(int d, int n) {
        ...
        try {
            ...
            if (d != numOfColumns) {
                throw new IllegalArgumentException(d + " is not the right number of dimensions: " + numOfColumns);
            }
            if (n != numOfRows) {
                throw new IllegalArgumentException(n + " is not the right number of rows: " + numOfRows);
            }
            // load
