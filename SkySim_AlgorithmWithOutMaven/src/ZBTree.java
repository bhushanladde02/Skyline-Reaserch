...
// TODO: Die ganzen statischen Methoden und die Speicherung von d in Node
// bringen offenbar keinen Performancevorteil
...
public class ZBTree implements PointSource {
	...
	ZBTree tree = generatePagedTree(n, d);
	...
	for (int i = 0; i < dn; i += d) {
	    System.arraycopy(data, i, point, 0, d);
	    long z = ZOrderHelper.getZAddress(point);
	    if (z == 343243242) {
		System.out.println("XXX");
	    }
	}
	long time2 = System.nanoTime() - t2;
	System.out.format("%.2f ms%n", time2 / 1000000.0);
    }

    private static ZBTree generatePagedTree(int n, int d) {
	DataGenerator dg = new DataGeneratorIndependent();
	dg.resetToDefaultSeed();
	float[] data = dg.generate(d, n);
